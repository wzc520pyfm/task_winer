...
Future<v_Album> createAlbum(String user_phone,String user_passw) async{
...
class v_Album{
  ...
  v_Album({this.user_phone,this.user_passw});
          ...
          child: VerificationBox(
            ...
            onSubmitted: (value){
              ...
              if(value.toString()==widget.result.user_ver.toString()){
                ...
                setState(() {
                  _v_futureAlbum=createAlbum(widget.result.user_phone.toString(),widget.result.user_passw.toString());//执行网络请求
                  //后端收到数据后把数据存入数据库
                });
                ...
                //   print('error');
                // });
                setState(() {//奇怪的问题,这三行代码应该是在验证码正确时执行,测试完后应修改此bug
                  _v_futureAlbum=createAlbum(widget.result.user_phone.toString(),widget.result.user_passw.toString());//执行网络请求
                  //后端收到数据后把数据存入数据库
                });
              }
